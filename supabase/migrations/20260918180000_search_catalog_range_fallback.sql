-- search_catalog — repli PAR PLAGE en plus du repli par établissement (lot D, suite du
-- 2026-09-18 : « regarde comment mieux faire au niveau de Lobby et le cron »).
--
-- Le repli existant (20260917150000) est par ÉTABLISSEMENT : `e.lobby_last_synced_at`, une seule
-- colonne, mise à jour par la DERNIÈRE synchronisation réussie, quel que soit le mois. Un
-- établissement dont le mois 0 vient de réussir a donc un `lobby_last_synced_at` frais — même si le
-- mois 6 (qui vient d'entrer dans l'horizon, ou dont le sync échoue en boucle depuis le lot C) n'a
-- LITTÉRALEMENT jamais été écrit. Une recherche sur ce mois trouvait alors zéro ligne dans le
-- miroir, sans déclencher aucun repli (l'établissement n'est pas « périmé », juste incomplet pour
-- CETTE plage) — et le logement disparaissait à tort.
--
-- Une seule ligne ajoutée : `not exists (... m2.date between p_desde and p_hasta)`, SANS le filtre
-- `available_units > 0` du premier `exists` — c'est ce qui distingue « jamais synchronisé » (aucune
-- ligne du tout, réel inconnu, on retombe en optimiste) de « synchronisé et complet » (des lignes à
-- 0, un vrai signal qu'il ne faut surtout pas ignorer).
--
-- Procédure obligatoire pour cette fonction de 316 lignes (.claude/rules/supabase.md §7) :
-- `pg_get_functiondef` puis remplacement par occurrence COMPTÉE (exactement 1), jamais retapée.
      ...
      or (
        case
          ...
          when e.lobby_connector_active and p.type = 'lodging' and p.lobby_category_id is not null then
            ...
            -- LOT D (20260918180000). L'établissement peut être frais DANS L'ENSEMBLE (un
            -- autre mois vient de réussir) alors que CE mois précis n'a jamais été
            -- synchronisé — horizon qui vient de s'ouvrir (le rang 6 n'entre qu'une fois
            -- par jour), ou couple en échec répété (fail_pms_sync, 20260918170000). La
            -- fraîcheur par établissement ne peut pas le voir : elle ne regarde qu'UNE
            -- colonne, jamais quel mois est derrière. Repli PAR PLAGE : aucune ligne DU
            -- TOUT pour ce (établissement, catégorie, plage) — jamais une ligne à
            -- available_units = 0, qui reste un COMPLET réel et ne doit surtout pas être
            -- ignorée (c'est pour ça que ce n'est pas le même prédicat que le exists()
            -- ci-dessus, qui filtre déjà > 0).
            or not exists (
              select 1
              from public.pms_availability_mirror m2
              where m2.establishment_id = p.establishment_id
                and m2.lobby_category_id = p.lobby_category_id
                and m2.date between p_desde and p_hasta
            )
...
comment on function public.search_catalog(text, text[], text, boolean, int, date, date, int, int, int) is
  ...
  'miroir de l''établissement a plus de six heures. Depuis 2026-09-18 (lot D), échec OUVERT AUSSI '
  'par plage : aucune ligne du tout pour (établissement, catégorie, p_desde..p_hasta), même si '
  'l''établissement est frais par ailleurs (mois nouvellement entré dans l''horizon, ou en échec '
  'répété). Les AUTRES produits d''un établissement connecté restent non filtrés par dates, comme '
  'avant.';
