-- Parité proposition socio pour group_discount_threshold_qty/group_discount_pct (camp), migration
-- précédente 20260914130000. Sans ce lot, un prestataire configurerait la remise dans son
-- formulaire mais elle disparaîtrait silencieusement entre son écran et la proposition enregistrée
-- (whitelist par type), puis entre l'approbation admin et le produit final — précédent déjà vécu
-- avec `lodging_kind`/`unit_count` (cf. commentaires 2026-08-27 dans ces mêmes fonctions). 4 RPC
-- touchées, corps extraits par pg_get_functiondef depuis les versions vivantes (aucune migration
-- ultérieure ne les retouche), modifiés par occurrences comptées, signatures INCHANGÉES pour les 4
-- (create or replace seul suffit) :
--   1. submit_product_creation_proposal : les 2 clés rejoignent le bloc whitelist déjà dédié à
--      camp ('price_cop', 'duration_days').
--   2. create_product_from_proposal : les 2 colonnes rejoignent la liste insert into products.
--   3. submit_product_proposal (édition) : nouveau bloc whitelist dédié à camp — jusqu'ici ce
--      chemin n'avait aucun bloc spécifique à camp au-delà de default_capacity (partagé avec
--      activity/transport).
--   4. moderate_product_proposal, branche `elsif p_decision = 'approve'` (édition d'un produit déjà
--      publié) : écriture des 2 colonnes GARDÉE par `v_final_payload ? '<clé>'` (comme
--      external_booking_url/price_label déjà dans cette fonction) — une proposition `pending` créée
--      AVANT cette migration ne porte pas ces clés ; une écriture inconditionnelle effacerait un
--      group_discount déjà posé sur le produit au moment de l'approuver. La branche `kind = 'create'`
--      passe par create_product_from_proposal (point 2 ci-dessus), pas concernée ici.
...
-- Pas de nouveau grant nécessaire pour les 4 fonctions : signatures inchangées, les grants
-- existants restent valides pour ces mêmes objets function.
