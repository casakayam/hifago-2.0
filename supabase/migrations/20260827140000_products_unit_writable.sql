-- products.unit devient ÉCRIVABLE par l'application (défaut C4 du plan LobbyPMS).
--
-- LE DÉFAUT, et il est ancien. `unit` existe depuis 20260813190232 et n'est écrite NULLE PART par
-- l'application : seul seed.sql la renseigne. Tout logement créé par l'admin ou proposé par un
-- socio a donc `unit = NULL`, et l'unique lecteur — la fiche publique — n'affiche alors aucune
-- mention d'unité à côté du prix. La colonne était contrainte, documentée, lue… et morte.
--
-- Le 2026-08-27 elle a été ÉTENDUE à `per_house` (migration 20260827120000) : elle a cessé d'être
-- incomplète sans cesser d'être inerte. C'est ce second point que cette migration corrige.
--
-- CE QUI CHANGE ICI, ET C'EST TOUT : `unit` rejoint la whitelist des logements des deux RPC de
-- proposition, l'insert de create_product_from_proposal et l'update de moderate_product_proposal.
-- La colonne, sa contrainte et ses valeurs ne bougent pas.
--
-- POURQUOI PAS DE CONVERSION AUTOMATIQUE DEPUIS LOBBY. Le plan proposait « brancher la conversion
-- sur le type Lobby ». On ne le fait pas, et la raison est une observation du jour : les prix de
-- LobbyPMS sont par NIVEAU D'OCCUPATION (plans[].prices[] compte autant d'entrées que la capacité
-- d'une unité — GLAMPING capacity 2 → 2 prix, CAMPER Van capacity 4 → 4). Leur modèle n'est donc
-- pas le nôtre, et aucune correspondance mécanique n'existe : « CAMPER Van » est `privada` avec
-- `capacity: 4`, ce qui n'est ni per_person, ni per_two, ni per_house de façon évidente.
--
-- La liaison Lobby PROPOSE donc une valeur uniquement dans les cas NON AMBIGUS (cf.
-- packages/domain/src/products/lodgingUnit.ts) et ne propose RIEN sinon — plutôt que de deviner un
-- prix, ce qui est la donnée la plus visible d'une fiche. Le champ est éditable dans le formulaire,
-- exactement comme capacity et lodging_kind depuis l'arbitrage du 2026-08-26.
--
-- CE QUI N'EST DÉLIBÉRÉMENT PAS FAIT : le repli `NULL ⇒ per_person` du legacy
-- (src/services/catalogService.js:897) n'est PAS porté. L'appliquer ferait apparaître « por
-- persona » sur toutes les fiches existantes d'un coup, y compris celles où c'est faux. `NULL`
-- continue de signifier « pas de mention », ce qui est le comportement actuel et le seul sûr tant
-- que les fiches n'ont pas été revues une à une.
--
-- Règle du projet : jamais éditer une migration déjà appliquée. Les 4 fonctions sont recréées,
-- corps extraits MÉCANIQUEMENT de 20260827120000, seul changement l'ajout de `unit`.

-- ============================================================================================
-- 1. submit_product_creation_proposal (verbatim 20260827120000) + unit
-- ============================================================================================
...
-- ============================================================================================
-- 2. create_product_from_proposal (verbatim 20260827120000) + unit
...
-- ============================================================================================
-- 3. submit_product_proposal (verbatim 20260827120000) + unit
-- ============================================================================================
...
-- ============================================================================================
-- 4. moderate_product_proposal (verbatim 20260827120000) + unit
-- ============================================================================================
