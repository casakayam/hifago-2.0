-- Spec 15 (docs/specs/15-socio-creation-produit.md) — révision de la décision 3 (Jérôme,
-- 2026-08-17, même jour) : les photos rejoignent finalement la proposition de création
-- elle-même, au lieu d'être différées après approbation — conforme au cahier des charges socio
-- §3e ("jusqu'à 6 photos" dans la proposition de fiche classique), jamais construit jusqu'ici.
--
-- Chemin d'upload déjà générique aux deux rôles (apps/admin/app/api/upload/[entity]/route.ts,
-- commentaire de tête : "admin ET socio passent par CE MÊME Route Handler") — aucune nouvelle
-- route nécessaire, le socio uploade ses photos en Storage exactement comme l'admin-direct
-- (StagedProductPhotos, réactivé côté variant socio-proposal), seul le rattachement DB diffère
-- (ici, à l'approbation, pas au clic "Crear").
--
-- Le formulaire de modération (ModerateProductCreationProposalForm) n'édite jamais les photos
-- (aperçu lecture seule seulement) : `photos` provient donc TOUJOURS de la soumission ORIGINALE du
-- socio (v_proposal.payload), jamais d'un p_corrected_payload qui ne le porterait pas — sinon une
-- correction de l'admin sur un autre champ effacerait silencieusement les photos proposées.

-- ============================================================================================
-- 1. Extension de submit_product_creation_proposal — whitelist + plafond 6 photos
-- ============================================================================================
...
-- ============================================================================================
-- 2. Extension de create_product_from_proposal — rattache les photos proposées
-- ============================================================================================
...
-- ============================================================================================
-- 3. Extension de moderate_product_proposal — photos toujours issues de la soumission originale
-- ============================================================================================

-- Signature INCHANGÉE. Seule la branche kind='create' change : après avoir résolu
-- v_final_payload (corrigé ou proposé), les photos sont TOUJOURS reprises depuis
-- v_proposal.payload (jamais depuis p_corrected_payload, qui ne les porte pas — le formulaire de
-- modération n'édite jamais les photos, cf. commentaire de tête) — sinon une correction sur
-- n'importe quel autre champ effacerait silencieusement les photos proposées par le socio.
