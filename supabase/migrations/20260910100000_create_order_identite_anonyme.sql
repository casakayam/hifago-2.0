-- Spec 31 (Tranche 1) — identité anonyme : garde `not_authenticated` réintroduite dans
-- create_order (retirée le 2026-08-13, cf. commentaire au point d'appel ci-dessous pour pourquoi
-- ça ne renverse PAS cette décision), et le bloc d'attribution exclut désormais explicitement une
-- session anonyme des deux côtés (lecture ET écriture de `saved_attribution_code`) — invariant 3.
--
-- Checklist (CLAUDE.md §3/§4) : create_order reste `security definer` + `set search_path = ''`,
-- une seule fonction, un seul aller-retour réseau, verrouillage `for update` INCHANGÉ (aucune des
-- deux modifications ne touche le squelette anti-survente — ni le `for update` sur
-- `product_availability`/`provider_resources`, ni le décompte de capacité). Pas de nouveau test de
-- concurrence requis : rien du verrouillage n'a changé, seule la garde d'identité en tête et la
-- condition du bloc d'attribution, tous deux évalués AVANT toute lecture verrouillante.
--
-- Repris de pg_get_functiondef sur la base locale (ce qui tourne réellement) ; seules les 3 lignes
-- commentées « 2026-09-10 » diffèrent de la version en place.
...
begin
  ...
  elsif v_account_id is not null and not (select public.is_anonymous_session()) then
  ...
  if v_account_id is not null and not (select public.is_anonymous_session())
     and p_attribution_code is not null and v_referrer_partner_id is not null then
