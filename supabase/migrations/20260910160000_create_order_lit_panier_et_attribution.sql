-- Spec 32 (panier en base), Tranche 2 — create_order lit ses propres lignes (cart_items) et son
-- attribution (carts) côté serveur, au lieu de les recevoir en paramètres d'un client qui pourrait
-- les falsifier (prix/quantité/référent). Corps extrait par pg_get_functiondef depuis la version
-- vivante (supabase.md règle 7), modifié par occurrences comptées :
--   - p_lines (paramètre) renommé v_lines (variable), peuplée depuis cart_items pour auth.uid().
--   - p_attribution_code/p_attribution_source (paramètres, retirés de la signature) remplacés par
--     une lecture de carts.attribution_code/attribution_source pour auth.uid(), en PRIORITÉ sur le
--     repli existant partner_accounts.saved_attribution_code (compte réel, additif — rien retiré).
--   - Plafonds corrigés : 4/12/20 → 12/36/40 (cahier §3e, valeurs validées le 2026-09-07 pour la
--     commande multi-établissements, jamais codées jusqu'ici — trouvé en préparant spec 32 §10).
--   - cart_items/carts vidées pour ce compte juste après la création de la commande (jamais avant
--     un retour 'ok', false, qui laisse le panier intact — cas limite spec 32 §0).
-- Signature changée (perd deux paramètres) : drop function explicite, jamais un create or replace
-- qui retirerait un paramètre silencieusement (supabase.md règle 7).

drop function public.create_order(jsonb, text, text, text, boolean, text, text);
...
-- Grant explicite requis : une signature différente est un NOUVEL objet function pour Postgres,
-- reparti d'un privilège par défaut (piège supabase.md, sens inverse des tables) — même geste que
-- 20260817190000_orders_holder_email_required.sql:305, dernier précédent à avoir changé cette
-- signature.
grant execute on function public.create_order(text, text, text, boolean) to authenticated, anon;
