-- Gap découvert en session (produit jetski 884305ee-0a73-421e-8004-51be459b1469, socio confondait
-- Cantidad mínima/máxima — bornes PAR COMMANDE, product-type-fields.tsx — avec le cupo quotidien
-- réel, product_availability.capacity) : pour un produit générique (activity/camp/evento/
-- transport), AUCUN champ de capacité n'existe à la création — le cupo n'est configurable que
-- date par date, sur le calendrier, jamais en une fois. `calendar_default_open` permet déjà de ne
-- PAS configurer ouvert/fermé jour par jour (un défaut au niveau produit) ; rien d'équivalent
-- n'existe pour la capacité, donc même un produit "toujours ouvert" reste invendable tant qu'au
-- moins UNE ligne product_availability n'a pas été posée à la main.
--
-- Fix : products.default_capacity (nullable, symétrique de calendar_default_open) — tant qu'aucune
-- ligne product_availability n'existe pour une date, create_order matérialise désormais cette
-- ligne avec capacity=default_capacity/booked=0 AVANT son verrouillage Phase 2 habituel (donc
-- verrouillée et décrémentée normalement ensuite, aucune branche de calcul dupliquée). Portée
-- volontairement limitée à la branche historique (date unique, room_type_id et end_date tous deux
-- null) : chambre d'hôtel et alojamiento par plage gardent leurs propres modèles de capacité déjà
-- explicites (room_type_availability / réservation systématiquement accompagnée d'une plage), pas
-- concernés par ce gap.
alter table public.products add column default_capacity integer;
alter table public.products add constraint products_default_capacity_positive
  check (default_capacity is null or default_capacity > 0);

comment on column public.products.default_capacity is
  'Cupo quotidien par défaut (branche date unique uniquement), utilisé par create_order tant '
  'qu''aucune ligne product_availability explicite n''existe pour la date demandée. NULL = '
  'comportement historique inchangé (aucune date n''est réservable sans configuration explicite '
  'via set_product_availability/le calendrier).';

-- create_order (dernière version : 20260817220400) — AJOUT d'une étape de matérialisation avant
-- la boucle de verrouillage Phase 2 existante ; tout le reste de la fonction (Phases 1/2b/2c/3/4,
-- branches chambre/alojamiento/historique) reste au mot près identique à 20260817210000.
...
begin
  ...
  -- Phase 2 (nouveau, avant verrouillage) : matérialise une ligne product_availability par défaut
  -- pour toute ligne à DATE UNIQUE (room_type_id et end_date tous deux null — jamais les branches
  -- chambre/alojamiento, qui ont leurs propres modèles) dont le produit porte un default_capacity
  -- et n'a encore AUCUNE ligne pour cette date précise. Idempotent (on conflict do nothing) et
  -- neutre sur l'invariant anti-survente : capacity=default_capacity, booked=0 — même si la
  -- validation Phase 3 refuse ensuite la commande pour une autre raison, cette ligne matérialisée
  -- ne consomme aucun cupo, elle rend juste explicite un état par défaut déjà vrai implicitement
  -- (même logique que calendar_default_open pour l'ouverture). DOIT précéder la boucle de
  -- verrouillage ci-dessous : une fois la ligne insérée, le SELECT ... FOR UPDATE qui suit la
  -- trouve et la verrouille normalement — l'unicité (product_id, date) sérialise deux INSERT
  -- concurrents pour la même première réservation d'un jour jamais configuré (celui qui perd la
  -- course voit ON CONFLICT DO NOTHING puis verrouille la ligne posée par l'autre, aucune fenêtre
  -- de double-comptage).
...
-- modify_order_line (dernière version : 20260817220400) — même fallback appliqué à la branche
-- historique (date unique) : déplacer une ligne réservée vers une date jamais configurée doit
-- pouvoir réutiliser le default_capacity du produit exactement comme create_order, sinon la
-- création profiterait du défaut mais la modification resterait bloquée sur "aucune disponibilité
-- définie" — incohérence délibérément évitée. Signature inchangée (5 paramètres), donc CREATE OR
-- REPLACE suffit, aucun DROP requis.
