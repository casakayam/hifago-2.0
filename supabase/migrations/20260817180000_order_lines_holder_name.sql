-- Spec 17 §0 Tranche 1 (« Mis Reservas » socio) — order_lines.holder_name, dupliqué depuis
-- orders.holder_name, MÊME raison et MÊME patron que order_lines.account_id (déjà dupliqué,
-- 20260813194515_availability_orders_core_tables.sql : "évite qu'une lecture de order_lines
-- déclenche en cascade l'évaluation de la policy RLS d'orders").
--
-- Ici la raison est plus forte qu'une simple évaluation en cascade : orders_select
-- (20260813194515) n'autorise QUE is_admin()/l'acheteur — aucune policy operator dessus. Ouvrir
-- orders en lecture à un operator (symétrique de order_lines_select_operator) exposerait TOUTE la
-- ligne, y compris holder_phone/holder_email, à un client Supabase non restreint aux colonnes que
-- l'écran Next.js choisit d'afficher — RLS ne filtre jamais par colonne. Dupliquer seulement
-- holder_name (jamais phone/email) sur order_lines, déjà lisible par l'operator depuis
-- 20260817170000, respecte structurellement le "PII minimale, jamais téléphone ni email" du
-- cahier des charges socio, plutôt que de compter sur la seule discipline du SELECT applicatif.
alter table order_lines add column holder_name text;

update order_lines ol set holder_name = o.holder_name
  from orders o where o.id = ol.order_id;

alter table order_lines alter column holder_name set not null;

-- create_order (dernière version : 20260817160000_calendar_tranche0_fixes.sql) — Phase 4, insère
-- désormais holder_name (déjà disponible via p_holder_name, aucun nouveau paramètre) dans la même
-- écriture order_lines. Reste du corps identique mot pour mot.
