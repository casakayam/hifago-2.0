-- Suite de 20260818090000_product_default_capacity.sql : products.default_capacity (cupo
-- quotidien par défaut, activity/camp/transport) est déjà une colonne réelle et déjà consommé par
-- create_order/modify_order_line — ce lot branche le champ dans le parcours de PROPOSITION socio
-- (création ET édition), symétrique de `capacity` (lodging) déjà présent dans ces 4 fonctions.
--
-- Règle du projet (hifago/CLAUDE.md) : jamais éditer une migration déjà appliquée en place — les 4
-- fonctions ci-dessous sont recréées ici via `create or replace function`, corps copié VERBATIM
-- depuis leur dernière définition existante (grep confirmé : create_product_from_proposal/
-- submit_product_creation_proposal → dernière définition dans 20260817140000 ;
-- submit_product_proposal/moderate_product_proposal → dernière définition dans 20260817150000),
-- plus le seul ajout ciblé default_capacity par fonction. Signatures inchangées partout.
--
-- Périmètre : default_capacity ne concerne que activity/camp/transport (hasDefaultCapacity côté
-- apps/admin/lib/products/useProductTypeFieldsState.ts) — jamais le groupe lodging (qui a déjà
-- `capacity`, un concept différent : couchage total, pas cupo quotidien) ni evento/hotel.

-- ============================================================================================
-- 1. create_product_from_proposal — insère default_capacity depuis p_payload, miroir de capacity
--    (dernière définition : 20260817140000_product_creation_proposal_photos.sql, avec photos)
-- ============================================================================================
...
-- ============================================================================================
-- 2. submit_product_creation_proposal — whitelist default_capacity (activity/camp/transport),
--    sinon la valeur envoyée par buildProductCreationPayload est silencieusement filtrée avant
--    même d'atteindre create_product_from_proposal ci-dessus. Dernière définition :
--    20260817140000_product_creation_proposal_photos.sql (photos + plafond 6).
-- ============================================================================================
...
-- ============================================================================================
-- 3. submit_product_proposal (édition) — whitelist default_capacity (activity/camp/transport),
--    même raisonnement que le point 2 ci-dessus, côté édition d'un produit déjà créé. Dernière
--    définition : 20260817150000_product_edit_proposal_full_parity.sql.
-- ============================================================================================
...
-- ============================================================================================
-- 4. moderate_product_proposal — branche kind='content' (défaut) applique aussi default_capacity
--    sur l'update de products. Branches 'create'/'photos' inchangées (le corrigé de kind='create'
--    est un objet complet passé tel quel à create_product_from_proposal, déjà couvert par 1.).
--    Dernière définition : 20260817150000_product_edit_proposal_full_parity.sql.
-- ============================================================================================
