-- Feature 14 (Socio : dashboard de commissions, lecture seule).
--
-- Constat — accès manquant : order_lines_select/orders_select (Tranche 3) ne couvrent que
-- « propriétaire de la commande » et « admin ». Un référent n'est ni l'un ni l'autre — sous la RLS
-- actuelle, il ne peut lire aucune ligne qu'il a pourtant apportée. Nouvelle policy nécessaire, pas
-- un correctif d'un bug : personne n'avait encore besoin de cet accès avant cette feature.
--
-- order_lines.referrer_partner_id : dénormalisé depuis orders.referrer_partner_id, pas une
-- jointure dans la policy — même raisonnement déjà posé sur order_lines.account_id en Tranche 3
-- (« évite qu'une lecture de order_lines déclenche en cascade l'évaluation de la policy RLS
-- d'orders », checklist /hifago-migration point 2). Nullable comme orders.referrer_partner_id
-- lui-même (une commande directe n'a pas de référent).
alter table order_lines add column referrer_partner_id uuid references partners(id);

-- 6e évolution de create_order (Tranche 3 → feature 6 → correctif réservation invité → feature 7 →
-- feature 11 → ici). Signature INCHANGÉE : create or replace seul suffit, pas de drop. Un seul
-- ajout : referrer_partner_id = v_referrer_partner_id (déjà résolu plus haut dans la fonction, pour
-- orders.referrer_partner_id) sur le même insert into order_lines — rien d'autre ne change en
-- Phase 4.
...
-- Policy additive : Postgres combine plusieurs policies select permissives par OR — pas besoin de
-- toucher order_lines_select (Tranche 3, déjà testée). Aucun accès à orders nécessaire pour cette
-- feature : produit/date/montant/part référent/statut vivent déjà sur order_lines — pas de
-- coordonnées du client exposées (même prudence que la restriction "jamais le contact client" déjà
-- appliquée au prestataire, cf. plan feature 14).
create policy order_lines_select_referrer on order_lines
  for select
  using (referrer_partner_id = (select partner_id_for_account(auth.uid())));
