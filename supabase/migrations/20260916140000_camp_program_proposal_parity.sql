-- Parité proposition socio pour products.program (camp), migration précédente 20260916130000.
-- Sans ce lot, un prestataire saisirait le programme dans son formulaire et il disparaîtrait
-- silencieusement entre son écran et la proposition enregistrée (whitelist par type), puis entre
-- l'approbation admin et le produit final — précédent déjà vécu avec lodging_kind/unit_count
-- (2026-08-27) puis group_discount_* (20260914140000, dont ce fichier est la copie stricte).
--
-- Jérôme a tranché le 2026-09-16 que le programme est éditable par l'admin ET par le socio
-- propriétaire, ce dernier via le circuit de proposition/modération — c'est précisément ce qui
-- impose une COLONNE de products plutôt qu'une table fille : le chemin d'ÉDITION
-- (submit_product_proposal) exclut structurellement les tables filles, cf. son propre commentaire
-- « jamais tags/photos/slot_rules ». Raisonnement complet en tête de 20260916130000.
--
-- 4 RPC touchées, corps extraits par pg_get_functiondef depuis les versions vivantes de la base
-- locale (aucune migration ultérieure ne les retouche), modifiés par occurrences comptées — chaque
-- remplacement vérifié unique avant écriture. Signatures INCHANGÉES pour les 4 (create or replace
-- seul suffit, cf. .claude/rules/supabase.md règle 7) :
--   1. submit_product_creation_proposal : 'program' rejoint le bloc whitelist camp
--      ('price_cop', 'duration_days', group_discount_*), + plafond program_cap_exceeded.
--   2. submit_product_proposal (édition) : même clé dans le bloc camp ouvert par la migration
--      précédente, + le même plafond.
--   3. create_product_from_proposal : colonne ajoutée à l'insert, valeur NORMALISÉE.
--   4. moderate_product_proposal, branche `approve` d'une édition : écriture GARDÉE par
--      `v_final_payload ? 'program'` ET normalisée.
--
-- ⚠️ LE POINT À NE PAS RATER — la normalisation (points 3 et 4). Les RPC lisent `p_payload ->
-- 'clé'` (flèche simple), donc un payload {"program": null} produit le littéral JSON `null`
-- ('null'::jsonb), PAS un NULL SQL. Comme jsonb_typeof('null'::jsonb) vaut 'null' et non 'array',
-- le CHECK products_program_is_array posé par 20260916130000 rejetterait alors TOUTE création de
-- camp sans programme (y compris /hifago-mock-data, dont le seeder écrit `item.x ?? null`). Ce
-- n'est pas une hypothèse : 20260818240000_fix_price_tiers_json_null.sql documente 11 produits de
-- la base locale pollués par ce littéral sur price_tiers. On applique donc ici le même garde-fou
-- que normalize_price_tiers, à la source plutôt qu'à la lecture — ce qui rend le CHECK tenable et
-- donne à cette colonne la garantie que price_tiers n'a jamais eue.
--
-- Le chemin admin direct (supabase.from("products").update({program: ...})) n'est PAS concerné :
-- PostgREST écrit un vrai NULL SQL.
