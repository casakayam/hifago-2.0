...
// `/[locale]/transportes/[categoria]` — LES OFFRES D'UNE CATÉGORIE (spec 29 §5b, généralisée
...
export async function generateMetadata(
  props: Omit<PageProps<"/[locale]/transportes/[categoria]">, "searchParams">
): Promise<Metadata> {
  const { locale, categoria: slug } = await props.params;
  const categoria = await resolverCategoria("transport", slug, locale as Locale);
  if (!categoria) return {};
  return metadataCategoria(categoria, "transport", locale as Locale);
}

export default async function CategoriaPage({
  params,
  searchParams,
}: PageProps<"/[locale]/transportes/[categoria]">) {
