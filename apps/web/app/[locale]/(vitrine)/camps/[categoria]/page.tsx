...
// `/[locale]/camps/[categoria]` — LES OFFRES D'UNE CATÉGORIE (spec 29 §5b, généralisée
...
export async function generateMetadata(
  props: Omit<PageProps<"/[locale]/camps/[categoria]">, "searchParams">
): Promise<Metadata> {
  const { locale, categoria: slug } = await props.params;
  const categoria = await resolverCategoria("camp", slug, locale as Locale);
  if (!categoria) return {};
  return metadataCategoria(categoria, "camp", locale as Locale);
}

export default async function CategoriaPage({
  params,
  searchParams,
}: PageProps<"/[locale]/camps/[categoria]">) {
