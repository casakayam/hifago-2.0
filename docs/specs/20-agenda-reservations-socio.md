...
| 9 | Statuts affichés dans l'agenda (pleins/atténués/masqués selon le statut) | **Partiellement câblé le 2026-08-24** (bug trouvé par Jérôme en testant manuellement : une réservation `Expirada` s'affichait dans l'agenda). La partie "masqués" est maintenant réelle : la requête `order_lines` de `/partner` (page.tsx) filtre `.in("status", ["reserved", "fulfilled", "no_show"])`, excluant `cancelled_by_client`/`cancelled_by_provider`/`expired`/`superseded`. La distinction visuelle "pleins vs atténués" entre `reserved`/`fulfilled` et `no_show` reste recommandée mais non câblée (les 3 restent affichés au même style) — non bloquant, ajustable sans impact structurel. |
