...
- **Le chemin des propositions SOCIO ne porte ni `external_booking_url` ni `price_label`** (`productEditPayload.ts`) — vrai AUSSI pour les eventos, donc défaut antérieur et non une régression : un socio ne peut pas modifier l'URL d'une vitrine qu'il a proposée. Le chemin admin direct, lui, les porte depuis le 2026-09-08 (spec 30 §10ter).
...
- ⚠️ **La spec 30 n'est PAS intégralement livrée : le décompte de couchages n'est affiché nulle part.** La migration `20260908201000_search_catalog_n_alojamientos` (313 l.) le calcule, `buscar.ts` le mappe, le type `TarjetaOferta` le porte, 14 fixtures le posent à `null` — et `TarjetaOferta.tsx` ne le rend pas, alors que la spec le nomme en §0 (l. 280) et en §7c (« gagne `nAlojamientos` »). Reste à faire : l'affichage + les libellés pluriels es/en. Trouvé par `/simplify` le 2026-09-08 ; l'entrée de journal de la Tranche 4 dit « intégralement livrée », c'est faux sur ce point.
