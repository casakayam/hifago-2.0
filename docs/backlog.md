...
- **`npm run test:db` est ROUGE en local** (constaté le 2026-09-07) — 4 fichiers pgTAP échouent sur données accumulées : `admin_audit_log` (3), `catalog_rls` (2), `product_availability_rpc` (1), `set_product_availability_socio` (1). Vérifié indépendant de tout changement récent. La suite ne donne donc AUCUN signal aujourd'hui. Corrigé par un `npm run db:setup` — qui efface une base partagée entre sessions, donc demande l'accord de Jérôme.
