...
- **Identité anonyme — Tranches 1+2+3 LIVRÉES le 2026-09-10** (`docs/specs/31-identite-anonyme.md`) : session anonyme au premier ajout au panier, `orders.account_id`/`order_lines.account_id` NOT NULL, et refus par défaut désormais un CONTRÔLE MÉCANIQUE (`security_definer_exposure.test.sql` cas 2, prouvé en réel contre une RPC fictive) — `update_my_account_profile`/`set_my_payout_account` refermées, seules RPC restées ouvertes au-delà de la liste blanche. Reste : Tranche 4 (purge à 30j, panier exclu du critère).
