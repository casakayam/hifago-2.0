...
- **Oracles de lecture, 4 fonctions d'identité restantes** (`is_admin`/`has_capability`/`has_admin_capability`/`partner_id_for_account`, exploit reproduit le 2026-09-10 ; ~~`establishment_slug_from_name`~~ **REFERMÉ le 2026-09-22**, `20260922110000`, aucun appelant RPC légitime) — uuid en PARAMÈTRE (jamais `auth.uid()`), `EXECUTE` à `anon`+`authenticated`. ⚠️ **Piste « revoke anon » INVALIDÉE, prouvé le 2026-09-22** : `products_select_public` (`sellable=true or is_admin(auth.uid())`) appelle `is_admin` même pour une session anonyme dès qu'une ligne a `sellable=false` — le revoke casse la lecture publique du catalogue (reproduit en transaction annulée). **Vrai correctif : retirer le paramètre, lire `auth.uid()` en interne** (100 % des appels le font déjà, grep exhaustif : ~40 sites SQL + 19 `.rpc()` TS) — chantier plus lourd qu'un grant, **arbitrage Jérôme requis (CLAUDE.md §10)** avant de le lancer.
