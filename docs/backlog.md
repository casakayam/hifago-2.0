...
- **Les e2e admin échouent encore à ~15/80, mais plus JAMAIS sur l'authentification** (2026-09-08). ⚠️ Le diagnostic précédent était FAUX : GoTrue ne refuse pas le secret TOTP en clair du seed — la panne était un `deadlock detected (40P01)` quand deux workers montaient en AAL2 sur le compte admin partagé, visible seulement dans `docker logs supabase_auth_*` (le client reçoit un « Unexpected failure » générique). Corrigé par un `pg_advisory_lock` autour de la connexion des comptes à TOTP partagé (`packages/e2e-support/src/auth.ts`). Restent des échecs d'INTERFACE, à diagnostiquer séparément. Correction structurelle non faite : un compte admin par worker (`AGENTS-PARALLELES.md` point 5).
