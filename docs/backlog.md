...
- **`?tag=<slug inconnu>` vide l'accueil** (revue du 2026-09-08, spec 28 §10quinquies) — le §0 promet « tag inconnu → ignoré », vrai pour `tipo` mais faux pour `tag`, et `BuscadorInicio` le reporte à chaque recherche donc la page ne se déverrouille plus. Non atteignable aujourd'hui (rien ne fabrique de lien `?tag=`). **À corriger dans le lot de la spec 29**, qui redéfinit `search_catalog` et doit donner des tags au seed — il n'en a aucun, donc « chercher par libellé de tag » (cahier §2a) n'est couvert par rien.
