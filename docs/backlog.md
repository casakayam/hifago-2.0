...
- **Panier en base — spec 32, Tranches 1+2 implémentées le 2026-09-10** (`docs/specs/32-panier-en-base.md`) : tables `carts`/`cart_items` + RLS, `create_order` lit désormais ses lignes et son attribution côté serveur (plafonds 4/12/20 → 12/36/40 corrigés au passage). Concurrence réelle prouvée (5/5 runs, 2 scénarios, `create_order.concurrency.mjs` restructuré — N acheteurs distincts, plus un seul compte partagé). Les 6 scripts de concurrence appelant `create_order` sont désormais tous réécrits et verts (`npm run test:concurrency` complet passé). `create_order.test.sql` (94→93 assertions, un cas devenu structurellement intestable — `product_not_found` inatteignable via `cart_items`, FK vers `products` — retiré, signalé pas contourné) adapté et vert, `npm run test:db` confirme qu'il n'apparaît plus dans les échecs. **Reste à faire avant de considérer create_order terminé** : 6 fichiers pgTAP (`create_order_pms_backed`, `date_range_booking`, `notification_payment_events`, `order_lines_holder_contact_operator`, `order_lines_referrer_rls`, `release_order_after_pms_refusal`) à adapter à la nouvelle signature. Tranches 3-4 (écran `/carrito`, `CartSummary`, `CartContext.tsx`, `CheckoutForm.tsx`) non commencées. 2 points ouverts restants (spec §10) : interaction avec `partner_accounts.saved_attribution_code`, mécanisme de revérification de dispo à la reprise du panier.
...
- **Chantier front vitrine** — les points ouverts du parcours client cible sont groupés dans `docs/01-cahier-des-charges-client.md` §2f (recherche géo différée, forme du voucher, dispo PMS en recherche datée, rattachement d'une commande invitée à un compte). ⚠️ « Source du contact d'un établissement » en est RETIRÉ : tranché le 2026-09-08 (colonne `establishments.contact_phone`, spec 30 §3.2) — `partners.phone` était le mauvais candidat, 0/36 rempli. « Plafonds du panier » en est RETIRÉ aussi (trouvé périmé le 2026-09-10 en préparant spec 32) : §3e du même cahier l'avait déjà tranché le 2026-09-07 (plafond global, valeurs 12/36/40/20) — §2f n'avait jamais été mis à jour le même jour. Le vrai point ouvert n'est pas la forme du plafond mais sa valeur, jamais codée (cf. spec 32 §10).
