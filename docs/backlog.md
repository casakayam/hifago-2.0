...
- **Paiement : garantie de confirmation LIVRÉE en local (spec 39, 2026-09-22)** — job `payments-reconcile` qui pilote l'expiration, libération des cupos, plafond 2 h, remboursement par le job, e-mails et écran client. ⚠️ Reste pour Jérôme : valider les textes des e-mails 9-10 (`docs/06`, spec 39 §5) et le trigger « drapeaux de capacité gelés » (§10.1). Reste pour Gabriel : déploiement préprod dans l'ordre de la spec 39 §C, capture des corps réels `/refunds`, mesure du délai d'indexation, un PSE réel.
...
- **Node 20 déprécié sur les runners GitHub** (avertissement à chaque run depuis le 2026-09-19) : `actions/checkout`, `actions/setup-node` et `gitleaks-action` sont épinglés par SHA sur des versions Node 20, déjà forcées en Node 24. À ré-épingler sur des releases Node 24 avant que le forçage ne cesse — un jour la CI tombera sans qu'aucun commit de ce dépôt n'ait bougé.
