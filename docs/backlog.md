...
- **Panier en base — spec 32, Tranches 1+2 ENTIÈREMENT TERMINÉES le 2026-09-10** (`docs/specs/32-panier-en-base.md`) : tables `carts`/`cart_items` + RLS, `create_order` lit désormais ses lignes et son attribution côté serveur (plafonds 4/12/20 → 12/36/40 corrigés au passage). Concurrence réelle (`npm run test:concurrency`, 8 fichiers) et suite pgTAP complète (`npm run test:db`, 54 fichiers/898 tests) toutes deux vertes — les 5 échecs restants sont préexistants, sans rapport (pollution `audit_log` déjà documentée). Un cas de test devenu structurellement intestable (`product_not_found`, `cart_items.product_id` porte une FK vers `products`) retiré, signalé en spec §10, pas contourné. **Reste : Tranches 3-4** — écran `/carrito`, `CartSummary`, `CartContext.tsx` (appels supabase-js directs), `CheckoutForm.tsx` simplifié — aucune ligne de code encore. 2 points ouverts restants (spec §10) : interaction avec `partner_accounts.saved_attribution_code`, mécanisme de revérification de dispo à la reprise du panier.
