...
- **Les e2e sont des bombes à retardement** (constaté le 2026-09-07) — `supabase/seed.sql` pose des disponibilités sur des dates EN DUR (`2026-09-05/07/10/12`) et huit specs codent les mêmes dates en dur. Celles du 05 et du 07 sont mortes aujourd'hui : 3 specs échouent. Les autres mourront d'ici le 15. Le helper qui règle ça existe déjà et est documenté pour ça (`isoDate()` de `packages/e2e-support`) — c'est le SEED qu'il faut rendre relatif.
...
- `createTtlCache` ne purge jamais les entrées expirées (Map non bornée) ; `addDaysIso` (`bogotaDates.ts:56`) porte le même débordement d'année déjà corrigé ailleurs.
- `partner-agenda.spec.ts` ne nettoie pas derrière lui, se sabote après plusieurs runs (accumulation locale).
- `pms_category_not_quoted` devrait rejoindre `connector_inactive` dans les motifs non retentables de `LodgingReservationForm.tsx` (laissé intact, une autre session l'éditait en même temps).
- Sonde à un appel LobbyPMS sur `start_date == end_date` pas encore vérifiée (verrue sans conséquence connue à ce jour).
