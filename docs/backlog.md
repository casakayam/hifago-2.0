...
- **Identité anonyme — Tranche 1 LIVRÉE le 2026-09-10** (`docs/specs/31-identite-anonyme.md`) : session anonyme au premier ajout au panier, garde `not_authenticated` sur `create_order`, invariant 3 (attribution jamais persistée sur une identité anonyme). Restent : Tranche 2 (compte technique `reserva-manual@hifago.local`, backfill, `orders.account_id`/`order_lines.account_id` NOT NULL, seed), Tranche 3 (liste blanche étendue aux ~80 autres RPC — une seule porte la garde aujourd'hui), Tranche 4 (purge à 30j, panier exclu du critère).
